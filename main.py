...
def encrypt(instring, shift, ls, dict): #takes input text, shift, and a list and dictionary(for language support)
...
def decrypt(cipher, shift, ls, dict): #takes cipher, shift, list and dict(lang support) to decipher
	cipher = cipher.split()
	out = ""
	for word in cipher:
		for letter in word:
...
if eord[0] == "e":
	try:
		print(encrypt(instring, shift, ls, dict))
	except:
		print("Only alphabetical characters and whitespace allowed.")
elif eord[0] == "d":
	try:
		print(decrypt(instring, shift, ls, dict))
	except:
		print("Only alphabetical characters and whitespace allowed.")
